...
EL2008::EL2008(const std::string& name, soem_interface::EthercatBusBase* bus, const uint32_t address) :
...
name_(name)
{
  ...
  pdoInfo_.txPdoSize_ = 0;//sizeof(reading_);
...
void EL2008::updateRead() {
  bus_->readTxPdo(address_, reading_);
}

void EL2008::updateWrite() {
